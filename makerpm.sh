...
WORKDIR="$TOPDIR/build/rpm"

cd "$TOPDIR"

BINARIES="expect rpmbuild rsync git"
...
{
    if use_git; then
        echo "  https://github.com/OpenNMS/jrrd2/commit/$(commit)"
    else
        echo ""
    fi
}
...
function main()
{
    ...
    if $BUILD_RPM; then
        ...
        run install -d -m 755 "$WORKDIR"/{BUILD,RPMS/$ARCH,SOURCES,SPECS,SRPMS}

        echo "=== Copying Source to Source Directory ==="
        run rsync -aqr --exclude=.git --exclude=build --exclude=target --delete --delete-excluded "$TOPDIR/" "$WORKDIR/tmp/$PACKAGE_NAME-$VERSION-$RELEASE/"

        echo "=== Creating a tar.gz archive of the Source in $WORKDIR/SOURCES ==="
        SPECS="rpm/jrrd2.spec"
    ...
    if $SIGN; then
        ...
        RPMS=$(echo "$WORKDIR"/RPMS/$ARCH/*.rpm)
    ...
    echo "==== jrrd2 RPM Build Finished ===="

    echo ""
    echo "Your completed RPMs are in the $WORKDIR/RPMS/$ARCH directory."
}
